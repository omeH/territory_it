...
namespace :api do
  namespace :tests do
    ...
    task :top_ratings, %i[url times processes] => :environment do |_, args|
      args.with_defaults(times: 5_000, processes: 5)

      times = args.times.to_i
      processes = args.processes.to_i
      Parallel.each(times.times, in_processes: processes) do
    ...
    task :authors, %i[url times processes] => :environment do |_, args|
      args.with_defaults(times: 5_000, processes: 5)
